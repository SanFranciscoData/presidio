...
AGENT_INSTALL_DIR = ".presidio-agent-install"
EGRESS_PROXY_SERVICE = "presidio-egress-proxy"
...
def install_step_script(step: InstallStep) -> str:
    """The install step's shell script with any per-step env exports inlined.

    Shared by every backend that materializes install steps (the docker-family
    Dockerfile bake and the E2B template builder) so they run byte-identical
    scripts."""
    if not step.env:
        return step.run
    exports = "".join(
    ...
    *,
    ...
    for step in install.steps:
        ...
        commands.extend(
            [
                f"USER {docker_user}",
                docker_run_command(install_step_script(step)),
            ]
        )
    return commands
...
def write_agent_dockerfile(
    *,
    ...
    dockerfile.extend(
        [
            f"ARG PRESIDIO_AGENT_INSTALL_FINGERPRINT={fingerprint}",
            docker_run_command(
                'printf "Presidio agent install fingerprint: %s\\n" '
                '"$PRESIDIO_AGENT_INSTALL_FINGERPRINT"'
            ),
        ]
    )
...
auth_param basic realm PresidioPolicyProxy
...
def write_docker_proxy_compose(
    *,
    ...
    compose = {
        "services": {
            "main": {
                "networks": ["presidio-egress-internal"],
                "depends_on": {
            ...
            EGRESS_PROXY_SERVICE: {
                ...
                "networks": ["presidio-egress-internal", "default"],
            },
        },
        "networks": {
            "presidio-egress-internal": {
                "internal": True,
            },
        },
    }
