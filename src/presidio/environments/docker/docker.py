...
from presidio.models.trial.config import ResourceMode, ServiceVolumeConfig
...
class DockerEnvironment(BaseEnvironment):
    ...
    # Per-image-name build serialization shared with the other build-from-scratch
    # backends (see ``environments/build_lock.py``): if multiple environments with
    # the same image name start concurrently, only one builds while the rest wait
    # for the cached image.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._is_windows_container:
            ...
            from presidio.environments.docker.docker_windows import WindowsOps

            self._windows_container_name = f"presidio-{uuid.uuid4().hex[:12]}"
            self._platform = WindowsOps(self, self._windows_container_name)
        else:
            from presidio.environments.docker.docker_unix import UnixOps
        ...
        if resolved_task_keys:
            presidio_keys = set(self._env_vars.to_env_dict(include_os_env=False).keys())
    ...
    def resource_capabilities(cls):
        from presidio.environments.capabilities import EnvironmentResourceCapabilities
    ...
    async def _run_docker_compose_command(
        ...
        if self._windows_container_name:
            env["PRESIDIO_CONTAINER_NAME"] = self._windows_container_name
    ...
    async def start(self, force_build: bool):
        ...
        if not self._use_prebuilt:
            async with self._build_locks(self.environment_name):
