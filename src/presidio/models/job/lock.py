...
from presidio.models.job.config import JobConfig, RetryConfig
...
from presidio.utils.env import sanitize_env_assignment
...
class PresidioLockInfo(BaseModel):
...
class JobLock(BaseModel):
    ...
    presidio: PresidioLockInfo = Field(default_factory=PresidioLockInfo)
    ...
    def _canonical_payload(self) -> dict:
        payload = self.model_dump(
            mode="json",
            exclude={"created_at", "presidio", "invocation"},
        )
...
def _get_presidio_info() -> PresidioLockInfo:
    return PresidioLockInfo(
        version=_get_presidio_version(),
        git_commit_hash=_get_presidio_git_commit_hash(),
        is_editable=_get_presidio_is_editable_install(),
    )


def _get_presidio_version() -> str | None:
    try:
...
def _get_presidio_git_commit_hash() -> str | None:
    direct_url_data = _get_presidio_direct_url_data()
    ...
    if not _is_presidio_editable_install(direct_url_data):
        return None
...
def _get_presidio_is_editable_install() -> bool | None:
    direct_url_data = _get_presidio_direct_url_data()
    if direct_url_data is None:
        return None
    return _is_presidio_editable_install(direct_url_data)


def _get_presidio_direct_url_data() -> dict | None:
    try:
        dist = distribution("presidio")
    except PackageNotFoundError:
        return None
...
def _is_presidio_editable_install(direct_url_data: dict) -> bool:
