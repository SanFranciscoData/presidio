...
def analyze_command(
    ...
    from presidio.analyze.analyzer import Analyzer
    from presidio.analyze.models import JobAnalyzeResult
    ...
    try:
        if _is_job_dir(path):
            ...
            console.print(
                ...
                f"(no job.log). "
                f"For task quality checks, use 'presidio check <task-dir>' instead.[/red]"
            )
            raise typer.Exit(1)
    except (FileNotFoundError, ValueError, RuntimeError) as e:
        console.print(f"[red]❌ {e}[/red]")
