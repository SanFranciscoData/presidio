...
adapted to presidio's Trial API (no pytest-asyncio, no with_default_user shim).
"""
...
from presidio.models.trial.config import TaskConfig as TrialTaskConfig
...
from presidio.models.trial.result import AgentInfo
...
def _make_env(mounted: bool) -> AsyncMock:
    ...
    # Docker-like env: fixed policy at start, no per-phase switching.
    env.capabilities.dynamic_network_policy = False
    # Lifetime seam is sync in the real interface; no provider TTL ceiling.
    env.set_min_lifetime = MagicMock(return_value=None)
    env.provider_max_lifetime_sec = MagicMock(return_value=None)
...
async def _run_trial(task_dir, trials_dir, fake_create):
    ...
    with (
        patch(
            "presidio.trial.trial.EnvironmentFactory.create_environment_from_config",
            side_effect=fake_create,
        ),
        patch(
            "presidio.trial.execution.AgentFactory.create_agent_from_config",
