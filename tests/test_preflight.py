...
async def test_egress_probe_checks_hosts_and_deletes_sandbox(monkeypatch, tmp_path):
    ...
    class FakeEnvironment:
        ...
        async def exec(self, command, timeout_sec):
            ...
            return ExecResult(return_code=0, stdout="204")
    ...
    assert environment.commands == [
        (
            "curl -sS --max-time 10 -o /dev/null -w '%{http_code}' https://api.example.com",
            15,
        )
    ]
    assert environment.deleted is True
    assert "api.example.com: pass (HTTP 204)" in output


@pytest.mark.anyio
async def test_egress_probe_treats_http_error_as_reachable(monkeypatch, tmp_path):
    task_config = SimpleNamespace(path=tmp_path)
    task = SimpleNamespace(
        name="task",
        paths=SimpleNamespace(environment_dir=tmp_path / "environment"),
    ...
    class FakeEnvironment:
        ...
        async def exec(self, command, timeout_sec):
            self.commands.append((command, timeout_sec))
            return ExecResult(return_code=0, stdout="403")
    ...
    assert "api.example.com: pass (HTTP 403)" in output
...
async def test_egress_probe_maps_host_failure(monkeypatch, tmp_path):
    ...
    class FakeEnvironment:
        ...
        async def exec(self, command, timeout_sec):
            return ExecResult(return_code=7, stderr="blocked")
