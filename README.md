...
Pick a sandbox backend with `--env` and pass backend-specific options with `--ek key=value`. Non-obvious Daytona behavior: sandboxes auto-stop after 60 idle minutes and are deleted on stop by default (override with `--ek auto_stop_interval_mins=... --ek auto_delete_interval_mins=...`), and custom labels can be attached with `--ek sandbox_labels={"k":"v"}`.
